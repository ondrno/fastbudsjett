...
def test_validate_test_returns_str(date, iso_expected):
    iso = validate_date(date)
    assert iso == iso_expected


def test_validate_raises_exception_if_wrong_date_format():
    with pytest.raises(ValueError):
        validate_date("05.12.20")


def test_validate_raises_exception_if_wrong_type():
    with pytest.raises(ValueError):
        validate_date(None)
