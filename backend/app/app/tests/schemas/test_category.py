...
def test_create_category_negative_itemtype_raises_exception(db: Session) -> None:
    name = random_string(length=10)
    with pytest.raises(ValidationError):
        CategoryCreate(name=name, itemtype_id=-1)
...
def test_update_category_with_negative_itemtype_raises_exception(db: Session) -> None:
    with pytest.raises(ValidationError):
        CategoryUpdate(itemtype_id=-1)
