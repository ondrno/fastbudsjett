...
def _create_schema_for_item(itemtype_id: int, category_id: int, payment_id: int) -> ItemCreate:
    return ItemCreate(description=DESCRIPTION, amount=AMOUNT, date=DATE,
                      category_id=category_id, payment_id=payment_id, itemtype_id=itemtype_id)


def _create_item(db: Session, itemtype_id: int, category_id: int, payment_id: int, user_id: int):
    item_in = _create_schema_for_item(itemtype_id, category_id, payment_id)
    return crud.item.create_with_owner(db=db, obj_in=item_in, owner_id=user_id)


def test_create_item(db: Session, test_itemtype, test_category, test_payment) -> None:
    user = create_random_user(db)
    item = _create_item(db, test_itemtype.id, test_category.id, test_payment.id, user.id)
...
def test_create_item_invalid_payment_id_raises_exception(db: Session, test_itemtype, test_category) -> None:
    user = create_random_user(db)
    with pytest.raises(ValidationError):
        _create_item(db, test_itemtype.id, test_category.id, -1, user.id)


def test_create_item_invalid_category_id_raises_exception(db: Session, test_itemtype, test_payment) -> None:
    user = create_random_user(db)
    with pytest.raises(ValidationError):
        _create_item(db, test_itemtype.id, -1, test_payment.id, user.id)


def test_get_item(db: Session, test_itemtype, test_category, test_payment) -> None:
    user = create_random_user(db)
    item = _create_item(db, test_itemtype.id, test_category.id, test_payment.id, user.id)
...
def test_update_item(db: Session, test_itemtype, test_category, test_payment) -> None:
    user = create_random_user(db)
    item = _create_item(db, test_itemtype.id, test_category.id, test_payment.id, user.id)
...
def test_update_item_invalid_category_id_raises_exception(db: Session, test_itemtype,
                                                          test_category, test_payment) -> None:
    user = create_random_user(db)
    item = _create_item(db, test_itemtype.id, test_category.id, test_payment.id, user.id)
...
def test_update_item_invalid_payment_id_raises_exception(db: Session, test_itemtype,
                                                         test_category, test_payment) -> None:
    user = create_random_user(db)
    item = _create_item(db, test_itemtype.id, test_category.id, test_payment.id, user.id)
...
def test_delete_item(db: Session, test_itemtype, test_category, test_payment) -> None:
    user = create_random_user(db)
    item = _create_item(db, test_itemtype.id, test_category.id, test_payment.id, user.id)
