...
def _create_schema_for_category(name) -> ItemCreate:
    return CategoryCreate(name=name)


def _create_category(db: Session, name: str):
    category_in = _create_schema_for_category(name)
...
def test_category_items_field_is_propagated_by_items(db: Session, test_payment) -> None:
    ...
    item_in = ItemCreate(description="test_description", amount="12.20", date="01.01.2020",
                         category_id=category.id, payment_id=test_payment.id)
